		...
		if o() then
			...
			elseif E == "AddGlobalSetHook" then
				...
				if J ~= nil then
					...
					local function L(M, N)
						g[l][t] = { Value = N, IsCallback = type(N) == "function" }
					end
					return { remove = K, Remove = K, modify = L, Modify = L }
				else
					l = l + 1
			...
			elseif E == "AddCallHook" then
				...
				local O = D[2]
				if type(O) ~= "function" or type(functionName) ~= "string" then
					error("mt-api: Invalid hook type")
				end
				if not d[self] then
					d[self] = {}
				...
				local function K()
					d[self][functionName] = nil
				end
				local function L(M, N)
					d[self][functionName] = { Callback = N }
				end
				return { remove = K, Remove = K, modify = L, Modify = L }
			elseif E == "AddGlobalCallHook" then
				...
				local function K()
					h[j][functionName] = nil
				end
				local function L(M, N)
					h[j][functionName] = { Callback = N }
				end
				return { remove = K, Remove = K, modify = L, Modify = L }
		...
		if not checkcaller() or getgenv().MTAPIDebug then
			...
			for w, x in next, h do
				if x[E] then
					local R = x[E]
					if R.Callback then
						return R.Callback(self, s, ...) or { Failure = true }
					end
					error("mt-api: Callback is nil")
					break
				end
...
local function S()
	...
	if getgenv().MTAPIGui then
		...
		game:AddGlobalCallHook("MouseButton1Click", function(self, T, ...)
		...
		game:AddGlobalCallHook("MouseButton2Click", function(self, T, ...)
